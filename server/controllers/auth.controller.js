...
// Controller function to get the profile of the currently logged-in user
const getMe = async (req, res) => {
  console.log(`Fetching profile for user ID: ${req.user.userId}`);
  try {
    // The userId is added to req.user by the protectRoute middleware
    const userId = req.user.userId;
    // Find the user by ID, excluding the password field
    const user = await User.findById(userId).select('-password'); 

    if (!user) {
      console.log(`GetMe failed: User not found with ID - ${userId}`);
      return res.status(404).json({ message: 'User not found' });
    }

    console.log(`Profile successfully fetched for user ID: ${userId}`);
    // Send the user data (without password) back to the client
